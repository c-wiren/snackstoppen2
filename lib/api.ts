...
type Result<T> =
  | { ok: true; value: T }
  | { ok: false; error: { error: string; type: "Client" | "Server" } };
...
async function client_auth<T extends Parameters<typeof client>[0]>(
  ...args: Parameters<typeof client<T>>
) {
  if (!args[2]) args[2] = {};
...
async function getSnacks(params: GetSnacksParams = {}) {
  ...
  let snacks: Snack[] = [];
  if (result.ok) {
    for (const snack of result.value) {
      snacks.push(onFetchSnack(snack));
    }
    return snacks;
  } else {
    throw result.error.error;
...
async function getUser(username: string): Promise<User | null> {
  const promise = (async () => {
    ...
    if (result.ok) {
      return onFetchUser(result.value);
    } else {
      if (result.error.error === "NotFound") {
        return null;
      } else {
...
async function getReviews(params: GetReviewsParams): Promise<Review[]> {
  ...
  if (result.ok) {
    for (const review of result.value) {
      reviews.push(onFetchReview(review));
    }
    return reviews;
  } else {
    throw result.error;
...
async function follow(user: User): Promise<Result<void>> {
  if (store.user && store.user.following !== undefined) {
