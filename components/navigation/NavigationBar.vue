    ...
    <header
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex flex-1 items-center">
          <!-- DESKTOP LOGO -->
          <NuxtLink to="/">
            <div class="ml-2 mr-6 hidden select-none font-semibold uppercase tracking-widest md:block">
              Snackstoppen
            </div>
          </NuxtLink>
          <!-- Removed  -->
        ...
        <!-- DESKTOP -->
        <nav class="hidden justify-center font-mono font-bold uppercase tracking-widest md:flex md:space-x-6">
          <NuxtLink v-for="item in nav" :key="item.name" :to="item.to"
            class="rounded px-2 py-1 transition hover:bg-white hover:bg-opacity-20 active:bg-opacity-40">{{ item.name }}
          </NuxtLink>
        </nav>
        <!-- MOBILE LOGO -->
        <NuxtLink to="/">
        ...
        <div class="ml-4 flex flex-1 items-center justify-end gap-2">
          <button class="rounded-md p-1 transition hover:bg-white hover:bg-opacity-20 active:bg-opacity-40"
            @click="state.overlay = 'search'">
            <MagnifyingGlassIcon class="h-6 w-6" />
          </button>
          <!-- USER -->
          <template v-if="api.currentUser">
            <button class="rounded-md p-1 transition hover:bg-white hover:bg-opacity-20 active:bg-opacity-40 md:px-2"
          ...
          <template v-else>
            ...
            <button class="rounded-md p-1 transition hover:bg-white hover:bg-opacity-20 active:bg-opacity-40 md:hidden"
              @click="state.overlay = 'login'">
      ...
      <!-- MOBILE -->
      <nav v-show="isOpen" ref="mobile-nav" class="flex h-screen flex-col justify-center gap-2 pb-44 md:hidden">
        <NuxtLink v-for="item in nav" :key="item.name" :to="item.to"
